...
Version `1.23.4-alexandria-beta.7` or newer. `agent` is a web-data subagent: it browses, searches, follows links, and returns either structured data or an answer. Hand it the question and let it do the browsing instead of chaining `search` and `scrape` calls yourself.
